...
import { useRelevance } from "../hooks/useRelevance.hooks";
...
function Relevance() {
  ...
  const { parameter, data, loading, error, totalPages, setData, setParameter } = useRelevance(
    param,
    pageNumber
  );
  ...
  useEffect(()=>{
    ...
    setData(filteredData.relevance)
  }, [])
        ...
        <div className="w-[30rem] flex flex-col items-center justify-center gap-2 px-3 py-5 border-2 rounded-xl">
          <div className="w-[25rem] h-[25rem] mb-[10rem]">
            <div className="text-right w-full flex justify-between items-center">
              <h2 className="font-medium text-3xl">RELEVANCE</h2>
              <select
            ...
            <div>
              <PieChart param1={parameter} param2={data} lebel={"Relevance"} />
            </div>
          </div>
