...
function useRelevance(param, pageNumber) {
    ...
    useEffect(() => {
        async function fetchData(url) {
            try {
                ...
                const likelihoodArray = rawData.map((p) => p['relevance'])
        ...
        if(param==="year" || "country") {
            const paramPageNumber = Number(pageNumber)+1
            if(paramPageNumber>0) {
                fetchData(`http://localhost:4400/api/v1/dashboard/relevance/${param}/${paramPageNumber}`)
            }
        }
